...
function version() {
	return "uct main v1.0";
}

class cvp {
	constructor(cv, format, txt, min="") {
		this.cv = cv;
		this.format = format;
		this.txt=txt;
		this.min=min;
...
function cvGas(elt, value=null) {
	...
	if (cvdatas !== undefined && !isNaN(cur_val)) {
		...
		if (cvdata !== undefined) {
				...
				var cval=cur_val*cvdata.cv;
				cur_val=utils.format(cvdata.format, cur_val);
				...
				nodes.forEach( function(elt_target){
					...
					if (cvdata !== undefined) {
						var targetval=cval/cvdata.cv;
						if (tre) {
							// if we convert Tre, there is an additional offset to add, defined in the temp_temp matrix
							var tt=temp_temp.get(elt_target.id);
							var ttoffset=tt.get(elt.id);
							targetval+=ttoffset;
						}
						elt_target.textContent=utils.format(cvdata.format, targetval);
					}
					else {
						console.log("could not find the entry corresponding to " + elt_target.id);
						console.log(elt_target);
					}
				});
				elt.textContent=cur_val;
				rv=true;
			}
		else {
			console.log("could not find the entry corresponding to " + elt.id);
			console.log(elt);
		}
	}
	return rv;
...
function getDescription (idc) {
	var cvdatas=all_cvdatas.get(idc.substr(0,2));
	var cvdata=cvdatas.get(idc);
	if (cvdata !== undefined) {
		return cvdata.txt;
   }
	return "";
